...
const DoctorFilterBar = ({ setEndPoint }) => {
  ...
  const SpecializationDropdown = () => {
    const handleSpecializationDropdown = (e) => {
      setFilters({ ...filters, specialization: e.target.value });
    };

    return (
      <select
        onChange={handleSpecializationDropdown}
  ...
  const FindBtn = () => {
    const onFindBtnClick = () => {
      ...
      const filtrationEndPoint = `https://dawiny-backend-48lm.vercel.app/api/v1/doctors?${filtersString}`;
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center gap-4 bg-white p-4 shadow rounded-lg">
      <div className="flex items-center mb-4 md:mb-0">
        ...
        <h2 className="text-lg font-semibold text-nowrap">Filter Doctors</h2>
      </div>
      <div className="flex-grow grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-2">
